...
final class LocalOverrideTest: QuickSpec {
    override class func spec() {
        ...
        describe("Local Overrides") {
            ...
            it("Gate Overrides") {
                overrides.setGate(user, FeatureGate.create("overridden_gate", true))
                let gate = statsig.checkGate("overridden_gate", user)
                expect(gate).to(equal(true))
            }
            
            it("DynamicConfig Overrides") {
                overrides.setDynamicConfig(user, DynamicConfig.create("overridden_config", ["key": "val"]))
            ...
            it("Experiment Overrides") {
                overrides.setExperiment(user, Experiment.create("overridden_exp", ["key": "val"]))
                let experiment = statsig.getExperiment("overridden_exp", user)
                expect(experiment.value["key"] as? String).to(equal("val"))
            }
            
            it("Layer Overrides") {
                overrides.setLayer(user, Layer.create("overridden_layer", ["key": "val"]))
